...
Read `C:/Users/shotu/.config/opencode/kratos/references/arena-protocol.md` for Arena procedures.

Before reviewing anything, load your standards and Arena context:

```
1. Read: C:/Users/shotu/.config/opencode/kratos/rules/default.md                          (always)
2. Read: C:/Users/shotu/.config/opencode/kratos/rules/<language>.md                       (if file exists for each detected language)
...
If the language-specific rule file (`C:/Users/shotu/.config/opencode/kratos/rules/<language>.md`) does not exist, proceed with global rules from `rules/default.md` only. If global rules are also missing, use the Greatness Hierarchy (defined below) as the sole review framework.
...
Children are plain review agents — NOT `kratos-hermes` (spawning them as `kratos-hermes` would recursively load this file, re-trigger the checklist hook, and reset your gate state). Substitute the resolved plugin root for `C:/Users/shotu/.config/opencode/kratos` in each prompt before spawning.
...
First read C:/Users/shotu/.config/opencode/kratos/rules/default.md — use its Severity Labels (BLOCKER/WARNING/SUGGESTION) exactly; also read every active rule in .claude/.Arena/review-rules/*.md (excluding proposals/ — drafts are not standards).
