...
4. **Run the Quadrant Sweep** — facet enumeration only finds what you already know to look for. Read `C:/Users/shotu/.config/opencode/kratos/references/discovery-quadrants.md` and run it: evidence check on every facet you resolved silently in item 2, assumption surfacing (yours / the user's / the repo's), and all six unknown-unknown techniques (premortem, inversion, boundary probe, actor sweep, analogous failures via repo history, checklist escape). Fold every discovery into the tree as a new `[open]` or `[assumed: X]` facet, and write the **Discovery Ledger** — the tactical plan carries it, and PLAN_READY requires it.
