...
Default: **normal**. If eco/power keywords are present (`eco`, `budget`, `cheap` / `power`, `max`, `full-power`), read `C:/Users/shotu/.config/opencode/kratos/modes/modes.md` for the full model matrix.
...
- Create minimal feature structure: `.claude/feature/<name>/`. If `.claude/feature/<name>/` does not exist, create it along with an initial status.json. See `C:/Users/shotu/.config/opencode/kratos/references/status-json-schema.md` for schema.
...
Use AskUserQuestion to determine where the decomposition should be output. A combo target (e.g. "local + notion") is reachable via the client's built-in "Other" free-text choice instead of a dedicated button. See `references/agent-protocol.md` § Interactive Questions.
