...
For new requests (not "continue" / "status"), read `C:/Users/shotu/.config/opencode/kratos/pipeline/classify.md` to determine intent and route correctly before proceeding.

### Step 1: Check Arena Staleness (Complex Tasks Only)

Read `C:/Users/shotu/.config/opencode/kratos/pipeline/check-arena-staleness.md` and execute its procedure.
...
Route on `action`:
- **`no-feature`** → use AskUserQuestion to ask what to build, then read `C:/Users/shotu/.config/opencode/kratos/pipeline/start.md`
...
- **`blocked`** → surface `reason` and `gate.failures[]` using the BLOCKED format; if the reason mentions recovery, read `C:/Users/shotu/.config/opencode/kratos/pipeline/recovery.md`

The CLI never picks among multiple features, never opts into optional stages, and reports default models only — those judgments (plus eco/power model overrides per `C:/Users/shotu/.config/opencode/kratos/modes/modes.md`) are yours.
...
| `procedure` | Action |
|-------------|--------|
| `spawn` | Spawn the listed agent(s) per `C:/Users/shotu/.config/opencode/kratos/pipeline/stages.md` |
| `spawn-parallel` | Spawn the listed agents in parallel (stage 9) |
| `gap-analysis` | Read `C:/Users/shotu/.config/opencode/kratos/pipeline/gap-analysis.md`, run the inline loop |
| `complexity-check` | Offer optional Stage 3 decompose / discuss, then proceed to stage 4 via the hephaestus gate |
| `hephaestus-gate` | Read `C:/Users/shotu/.config/opencode/kratos/pipeline/hephaestus-gate.md`, run the 3-phase gate |
| `pre-implementation` | Read `C:/Users/shotu/.config/opencode/kratos/pipeline/pre-implementation.md`, run the gate |
...
| `recovery` | Read `C:/Users/shotu/.config/opencode/kratos/pipeline/recovery.md` |
...
**You MUST read `C:/Users/shotu/.config/opencode/kratos/pipeline/stages.md` before spawning any agent.** Do not rely on memory of how stages work — the spawn prompts, phase flags, and gate procedures change between versions. Stage 1 and Stage 4 in particular have multi-step gate procedures that must be followed exactly as written.
...
|----------------|---------|------|
| *(new feature)* | — | **1-prd** — read `C:/Users/shotu/.config/opencode/kratos/pipeline/gap-analysis.md` and run the inline gap analysis loop. Do NOT spawn Athena with PHASE: GAP_ANALYSIS. |
...
| 3-decomposition | Complete/Skipped | **4-tech-spec** — read `C:/Users/shotu/.config/opencode/kratos/pipeline/hephaestus-gate.md` and run the 3-phase gate (Metis scan → Hephaestus ANALYZE → user questions → Hephaestus WRITE_SPEC). Do NOT spawn Hephaestus directly. |
...
Fallback (binary unavailable): before spawning any agent, verify the prior stage is complete and its deliverable file exists. See `C:/Users/shotu/.config/opencode/kratos/references/status-json-schema.md` for status.json schema and `C:/Users/shotu/.config/opencode/kratos/references/agent-handoff-spec.md` for agent contracts.

---

When a stage produces an unexpected verdict or the pipeline is stuck, read `C:/Users/shotu/.config/opencode/kratos/pipeline/recovery.md`.
