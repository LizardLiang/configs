...
1. **Spawn Hera** (PRD alignment check, stage 8):
   ...
   ```
    ...
    If Hera returns **aligned**, first check for a pending spec delta and offer to archive it (same procedure as Ares Mode — see `C:/Users/shotu/.config/opencode/kratos/pipeline/stages.md` Stage 8 section: "After Hera Returns: Spec Archive Offer"):
   ```bash
